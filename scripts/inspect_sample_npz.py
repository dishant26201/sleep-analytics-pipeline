...
epoch_start_point = data[
    'epoch_start_point'
]  # Access array with start points (in secs) for each epoch
...
print(
    "\nAll items in npz file".ljust(30), ": ", data.files
)  # View all the components in the npz file
...
plt.plot(
    time, X[epoch_id, 0, :], label=channels[0], color="red"
)  # All samples from the Fpz-Cz in that specific epoch
plt.plot(
    time, X[epoch_id, 1, :], label=channels[1], color="blue"
)  # All samples from the Pz-Oz in that specific epoch
plt.xlabel("Time (Seconds)")  # x-axis label
plt.ylabel("Amplitude (Volts)")  # y-axis label
plt.ticklabel_format(
    style="plain", axis="y"
)  # Prevent matplotlib from applying scientific notation on y axis voltage and shows full decimal values

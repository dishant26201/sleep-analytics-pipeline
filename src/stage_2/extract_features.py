...
def compute_psd(
    ...
    freqs, psd = welch(
        epoch, fs=sfreq, nperseg=nperseg, noverlap=noverlap
    )  # PSD of the whole epoch

    mask = (freqs >= min_freq) & (
        freqs <= max_freq
    )  # Apply mask with frequency range of our choice
...
def load_npz_data(npz_path: str | Path):
    ...
    return {
        ...
        "starts": data[
            "epoch_start_point"
        ],  # 1D NumPy array with start times of each epoch in seconds
...
def compute_time_domain_features(epoch: np.ndarray, channel: str):
    ...
    mobility = float(
        np.sqrt(temp1 / (activity + EPS))
    )  # Represents the mean frequency or the proportion of standard deviation
    complexity = float(
        np.sqrt(temp2 / (temp1 + EPS)) / (mobility + EPS)
    )  # Measures change in frequency or irregularity of the wave
...
def compute_freq_domain_features(epoch: np.ndarray, sfreq: float, channel: str):
    ...
    freqs, psd = compute_psd(
        epoch, sfreq, nperseg, noverlap, total_min, total_max
    )  # PSD of the desired frequency range with mask applied

    total_power = float(
        np.trapz(psd, freqs)
    )  # Computes area under PSD curve which gives total power in that frequency range
    ...
    for band_name, (low, high) in BANDS.items():
        freqs_band, psd_band = compute_psd(
            epoch, sfreq, nperseg, noverlap, low, high
        )  # PSD of the desired frequency range with mask applied

        absolute_power = float(
            np.trapz(psd_band, freqs_band)
        )  # Computes area under PSD curve of that band which gives total power in that band
        absolute_powers[band_name] = (
            absolute_power  # Add it to dictionary to store to compute features
        )

        relative_powers[band_name] = absolute_power / (
            total_power + EPS
        )  # % of power in that band compared to the total power and store in dictionary
